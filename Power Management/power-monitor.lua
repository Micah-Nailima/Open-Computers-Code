...
--Added some modifications to make it work with the new IC2 format, IE Capacitors and Flux Networks
...
local function readCapacity(proxy, ltype)
  ...
  if ltype == 4 then --For Flux Networks
    capacity = proxy.getFluxInfo().maxStorage
  end
...
local function readStored(proxy, ltype)
  ...
  if ltype == 4 then
    capacity = proxy.getFluxInfo().energyStored
  end
...
{id="ic2_te_mfsu", type=1, name="MFSU"},
{id="ic2_te_mfe", type=1, name="MFE"},
{id="ic2_te_cesu", type=1, name="CESU"},
{id="ic2_te_batbox", type=1, name="BatBox"},
...
{id="ie_lv_capacitor", type=2, name="LV Capacitor"},
{id="ie_mv_capacitor", type=2, name="MV Capacitor"},
{id="ie_hv_capacitor", type=2, name="HV Capacitor"},
{id="ie_creative_capacitor", type=2, name="Creative Capacitor"},
{id="flux_storage", type=4, name="Flux Storage"}
}
  ...
  for cid, cobj in pairs(controllers) do
      ...
      if cobj.type == "br_reactor" and status.activeCooling == false then
          table.insert(preactordata, {string.sub(cid, 8) , status.active, pad(round(status.fuelTemperature),4) .. "°C", 
            round(status.controlRodLevel) .. "%", pad(round(status.rate), 5).. "RF/t", superlib.pgen(status.energyStored, 10000000, config.display_precision) .. "%"})
          total_reactor_rate = total_reactor_rate + status.rate
      end
